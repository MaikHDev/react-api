...
export default function SearchBar() {
    ...
    const getApi = async () => {
        await ApiCall({
            parameters: `asset/v1/search?search_string=${query != null ? query : ''}&limit=10`,
        }).then((data) => {
            ...
            {apiData && (
                <ul className="mt-2">
                    {apiData.Data.LIST?.map((item) => (
                        <Link key={item.SYMBOL} href={`/api/${item.SYMBOL}`}
                              className="bg-slate-500 px-3 py-2 cursor-pointer relative flex hover:bg-slate-600">
                            {item.NAME}
                        </Link>
                    ))}
                    {apiData.Data.LIST && apiData.Data.LIST.length > 0 && (
                        <input type="submit"
