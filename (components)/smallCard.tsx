    ...
    return (
        <div className="flex w-[1000px] gap-3">
            <Link href={`/api/${item.SYMBOL}`}
                ...
                {item.PRICE_USD && (
                    <li className="">{item.PRICE_USD.toFixed(2)}$</li>
                )}
            ...
            <button
                className={`p-4 ${favorite ? ' bg-red-500 hover:bg-red-600 ' : ' bg-gray-500 hover:bg-gray-600 '} text-black mt-2 mb-2 rounded `}
                onClick={() => setFavorite(item.SYMBOL)}>{favorite ? 'Unfavorite' : 'Favorite'}</button>
        </div>
    );
}